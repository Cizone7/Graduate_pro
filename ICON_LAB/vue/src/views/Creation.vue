<template>
  <div>
    <div style="">
      <el-input style="width: 200px" placeholder="请输入作品名称" suffix-icon="el-icon-search" v-model="name"></el-input>
    ...
    <el-card>
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange" :header-cell-style="{'padding-left':'4px','padding-right':'4px'}">
        ...
        <el-table-column label="上传用户">
          <template v-slot="scope">
            <span v-if="scope.row.userId">{{ users.find(v => v.id === scope.row.userId) ? users.find(v => v.id === scope.row.userId).nickname : ''  }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="time" label="上传时间"></el-table-column>
        <el-table-column label="作品文件">
          <template v-slot="scope">
            <el-image style="width: 120px; height: 120px;"  :src="scope.row.img" :preview-src-list="[scope.row.img]"></el-image>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    load() {
      this.request.get("/creation/page", {
        params: {
    ...
    save() {
      this.request.post("/creation", this.form).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/creation/del/batch", ids).then(res => {
        if (res.code === '200') {
